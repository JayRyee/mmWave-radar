...
for frame_index = (start_frame+offset): 5: (end_frame - block_size)
    udCoM = zeros(1, 30);
    udUpperEnv = zeros(1,30);
    udLowerEnv = zeros(1,30);
    ...
    for blockCnt = 1:30
        ...
        UpperEnv = max(currDoppler - udCoM(blockCnt))
        
        udUpperEnv(blockCnt) = UpperEnv;
...
%writematrix(feature_arr, 'feature_arr_s2_145_260.csv')
